...
PathResult PathFinder::tryDirectPath(
    ...
    if (start.x != goal.x && start.y != goal.y) {
        return result;  // Not aligned
    }

    // For direct path: first segment = last segment
    // So exclude both sourceNode and targetNode at appropriate endpoints
    if (validateSegmentWithEndpoints(start, goal, obstacles, sourceNode, targetNode, true, true,
                                     extraStartExcludes, extraGoalExcludes)) {
        result.found = true;
        result.path = {start, goal};
        result.bendCount = 0;
        result.totalCost = manhattanDistance(start, goal);
    }
...
PathResult PathFinder::tryLShapedPath(
    ...
    // L-path has 2 segments:
    // Segment 0: first segment (exclude sourceNode from start)
    // Segment 1: last segment (exclude targetNode from goal)

    // Try horizontal first, then vertical
    GridPoint corner1 = {goal.x, start.y};
    bool hv_valid = validateSegmentWithEndpoints(start, corner1, obstacles, sourceNode, targetNode,
                                                  true, false, extraStartExcludes, extraGoalExcludes) &&
                    validateSegmentWithEndpoints(corner1, goal, obstacles, sourceNode, targetNode,
                                                  false, true, extraStartExcludes, extraGoalExcludes);

    // Try vertical first, then horizontal
    GridPoint corner2 = {start.x, goal.y};
    bool vh_valid = validateSegmentWithEndpoints(start, corner2, obstacles, sourceNode, targetNode,
                                                  true, false, extraStartExcludes, extraGoalExcludes) &&
                    validateSegmentWithEndpoints(corner2, goal, obstacles, sourceNode, targetNode,
                                                  false, true, extraStartExcludes, extraGoalExcludes);

    if (hv_valid) {
        result.found = true;
        result.path = {start, corner1, goal};
        result.bendCount = 1;
        result.totalCost = manhattanDistance(start, goal);
    } else if (vh_valid) {
        result.found = true;
        result.path = {start, corner2, goal};
        result.bendCount = 1;
        result.totalCost = manhattanDistance(start, goal);
    }
