...
/// 
/// In Separated mode:
///   - Incoming edges use local indices [0, inCount) mapped to range [0.0, 0.5)
///   - Outgoing edges use local indices [0, outCount) mapped to range [0.5, 1.0)
///   - Unified indices store: incoming [0, inCount), outgoing [inCount, inCount+outCount)
///
/// In Unified mode:
...
public:
    ...
    /// Get snap ranges for Separated mode
    /// @param connections Connection info for the node edge
    /// @return Pair of (incoming range, outgoing range)
    static std::pair<SnapRange, SnapRange> getSeparatedRanges(const NodeEdgeConnections& connections);
    ...
    /// Get string representation of snap mode
    static const char* modeName(SnapDistribution mode);
