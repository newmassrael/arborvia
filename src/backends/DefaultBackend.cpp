...
DefaultBackend::DefaultBackend() : currentLevel_(LogLevel::Debug) {
    ...
    if (!env_level) {
        env_level = std::getenv("SPDLOG_LEVEL");  // Fallback for compatibility
    }
    ...
    if (env_level) {
        ...
        if (level_str == "trace") currentLevel_ = LogLevel::Trace;
        else if (level_str == "debug") currentLevel_ = LogLevel::Debug;
        else if (level_str == "info") currentLevel_ = LogLevel::Info;
        else if (level_str == "warn" || level_str == "warning") currentLevel_ = LogLevel::Warn;
        else if (level_str == "err" || level_str == "error") currentLevel_ = LogLevel::Error;
        else if (level_str == "critical") currentLevel_ = LogLevel::Critical;
        else if (level_str == "off") currentLevel_ = LogLevel::Off;
    }
}

void DefaultBackend::log(LogLevel level, const std::string& message,
